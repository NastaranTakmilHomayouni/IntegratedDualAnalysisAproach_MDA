...
function update_dimensions_order_regarding_Filters() {
    ...
    let new_dimensions = [];
    dimensions_brushed_filtered.forEach(function (dim) {
...
function brush() {
    ...
    actives_copy.forEach(function (copy) {
        if (!dimensions.includes(copy.dimension)) {
    ...
    setTimeout(function () {
        ...
        function set_statistics_background_and_brush(dim) {
            ...
            d3.select('#' + dim).select('.' + id_statistics_background)
                .style('fill', statistics_scale(column_values_filtered.find(x => x.id === dim).descriptive_statistics[sort_parallel_coordinates_by]));
            ...
            if (dimensions_to_add.indexOf(dim) > -1 && actives_copy.filter(x => x.dimension === dim).length > 0) {
                ...
                //set_brush_programmatically(dim, [actives_copy.filter(x => x.dimension === dim)[0].extent[0], actives_copy.filter(x => x.dimension === dim)[0].extent[1]], false);
            }
        }
...
function expand_group(current_dimension) {
    ...
    if (min_contri_x === max_contri_x || range_axis < 0) {
        ...
        d3.select('#' + current_dimension.id).select('.' + id_statistics_background)
            ...
            .style('transform', 'translate(0px,' + (pcp_new_height - range_axis - bottom_buttons_width / 2 + 5 ) + 'px)');
        ...
        function update_position_path_ticks(element, tickpos) {
            element
                .transition()
                .duration(duration_time)
                .style('transform', 'translate(0px,' + (pcp_new_height - range_axis - bottom_buttons_width / 2 + 5 + tickpos) + 'px)');
        }
        ...
        function stay_position_bottom_views(class_) {
            d3.select('#' + current_dimension.id).selectAll('.' + class_)
                .transition()
                .duration(duration_time)
                .style('transform', 'translate(-140px,118px)rotate(-90deg)');
        }
    }
}

...
function initialize_dimensions_plots() {
    ...
    for (let i = 0; i < dimension_plot_preferences.length; i++) {
        ...
        initialize_axis_changing_button(dimension_plot_preferences[i].id, id_axis_changing_button, 7, "resources/missing_data.png", id_missing_values_integration, i, dimension_plot_preferences[i].all_descriptive_statistical_measures);
...
function add_legend(div_heading) {
    ...
    append_circle_and_text(0, color__numerical_data, id_data_type__numerical, "none", 1, radius_dimension);
    append_circle_and_text(1, color__categorical_data, id_data_type__categorical, "none", 1, radius_dimension);
    append_circle_and_text(2, color__date_data, id_data_type__date, "none", 1, radius_dimension);
    append_circle_and_text(2.8, color__numerical_data, language_id_opacity_missing_data, "none", 0.3, radius_dimension);
    ...
    div_heading.select('svg').append('line')
        .attr('x1', 4 * 200 + space_to_left)
        .attr('y1', 10)
        .attr('x2', 4 * 200 + 15 + space_to_left)
        .attr('y2', 10)
        .style('stroke', '#D3D3D3')
    ...
    div_heading.select('svg').append('text')
        ...
        .attr('transform', 'translate(' + (4 * 200 + 25 + space_to_left) + ',' + (33 / 2) + ')');
...
function initialize_axis_changing_button(parent_div_id, button_id, index_button, resource, axis_id, index_dimension_plot_preferences, all_descriptive_statistic_measures) {
    ...
    svg_append_filter_button.on('click', function () {
        ...
        if (axis_id === id_missing_values_integration) {
            include_missing_values_bool = !include_missing_values_bool;
            toggle_include_missing_values(function (response) {
                update_dimension_views();
            }, brush());

            let fill_color = getComputedStyle(axis_changing_div.node()).getPropertyValue(id_view_propertiy_dimensions_background_color);
...
function initialize_dimension_scatterplot(parent_div_id, data_type, x_axis_key, y_axis_key, possible_correlations_bool) {
    ...
    let all_circles = svg.selectAll('circle')
        ...
        .on("mouseover", function (d) {
            let y_key = y_axis_key;
            if (possible_correlations_bool) {
                y_key = key_data_type;
            }
            append_tooltip_radar_chart_and_heading(parent_div_id, d, false);
            overplotting_tooltip(d, xScale, yScale, x_axis_key, y_key, this);
        })
        .on("mouseout", function () {

            d3.select(this).style('opacity', function (d) {
                let opacity = 1 - column_values_filtered.filter(col => col[key_id] === d[key_id])[0].descriptive_statistics.missing_values_percentage;
                if (possible_correlations_bool) {
                    opacity = column_values_filtered.filter(col => col[key_id] === d[key_id])[0][key_column_values].filter(x => x !== null).length / column_values_cleaned.find(col => col[key_id] === d[key_id])[key_column_values].length + min_opacity/2;
                }
